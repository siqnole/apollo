...
async function problemRoutes(fastify) {
    ...
    `;
        ...
        if (difficulty) {
            query += ` AND p.difficulty = $${i++}`;
            params.push(difficulty);
        }
        if (type) {
            query += ` AND p.problem_type = $${i++}`;
            params.push(type);
        }
        if (search) {
    ...
    fastify.get('/problems/:slug', async (req, reply) => {
        const { slug } = req.params;
        const problemResult = await fastify.db.query(`SELECT * FROM problems WHERE slug = $1 AND active = true`, [slug]);
        if (problemResult.rows.length === 0)
            return reply.status(404).send({ error: 'Problem not found' });
        const problem = problemResult.rows[0];
        const [testCases, options] = await Promise.all([
            fastify.db.query(`SELECT id, input, expected_output, is_hidden, explanation, display_order
         FROM test_cases WHERE problem_id = $1 ORDER BY display_order`, [problem.id]),
            fastify.db.query(`SELECT id, label, body, display_order, is_correct
         FROM problem_options WHERE problem_id = $1 ORDER BY display_order`, [problem.id]),
        ]);
    ...
    fastify.post('/run', {
        ...
        const { language, code, input, fn_name, debug_mode } = req.body;
        if (!code || !language)
            return reply.status(400).send({ error: 'code and language required' });
        try {
            const result = await (0, judge_1.runCodeRaw)(code, language, input ?? '', fn_name ?? null, debug_mode ?? false);
            return reply.send({
    ...
    fastify.post('/submissions', {
        ...
        const { userId } = req;
        const { problem_slug, language, code, answer, fn_name } = req.body;
        const problemResult = await fastify.db.query(`SELECT * FROM problems WHERE slug = $1`, [problem_slug]);
        if (problemResult.rows.length === 0)
            return reply.status(404).send({ error: 'Problem not found' });
        const problem = problemResult.rows[0];
        const [testCases, options] = await Promise.all([
        ...
        const subResult = await fastify.db.query(`INSERT INTO submissions (user_id, problem_id, problem_type, language, code, answer, status)
       VALUES ($1, $2, $3, $4, $5, $6, 'running')
       RETURNING id`, [userId, problem.id, problem.problem_type, language ?? null, code ?? null, answer ?? null]);
        ...
        try {
            if (problem.problem_type === 'code' || problem.problem_type === 'debug') {
                ...
                if (!tc)
                    return reply.status(500).send({ error: 'No test case configured for this problem' });
                const sqlResult = await (0, sqlJudge_1.judgeSql)(fastify.db, tc.input, // setup SQL
                ...
                const correct = options.rows.find((o) => o.is_correct);
                judgeResult = (0, judge_1.judgeMultipleChoice)(answer, correct?.label ?? '');
            }
            else if (problem.problem_type === 'fill_blank') {
                if (!answer)
                    return reply.status(400).send({ error: 'answer required' });
                const expected = testCases.rows[0]?.expected_output ?? '';
                judgeResult = (0, judge_1.judgeFillBlank)(answer.trim(), expected.trim());
            }
                ...
                const correct = options.rows
                    ...
                    .map((o) => o.body);
                judgeResult = (0, judge_1.judgeOrdering)(submitted, correct);
            }
            else if (problem.problem_type === 'html_css') {
                if (!code)
                    return reply.status(400).send({ error: 'HTML code required' });
                const htmlResult = (0, htmlJudge_1.judgeHtml)(problem.slug, code);
                judgeResult = {
            ...
            fastify.log.error(err);
            judgeResult = { status: 'runtime_error', output: 'Internal judge error', runtime_ms: 0, results: [] };
        }
        // ── Award XP on first-time acceptance ─────────────────────────────────
        let xpAwarded = 0;
        if (judgeResult.status === 'accepted') {
            const prev = await fastify.db.query(`SELECT id FROM submissions WHERE user_id = $1 AND problem_id = $2 AND status = 'accepted' AND id != $3`, [userId, problem.id, submissionId]);
        ...
        // ── Update submission ──────────────────────────────────────────────────
        await fastify.db.query(`UPDATE submissions SET status = $1, output = $2, expected = $3, runtime_ms = $4, test_results = $5, xp_awarded = $6 WHERE id = $7`, [judgeResult.status, judgeResult.output, judgeResult.expected ?? null, judgeResult.runtime_ms, JSON.stringify(judgeResult.results ?? []), xpAwarded, submissionId]);
        return reply.send({
            id: submissionId,
            status: judgeResult.status,
            output: judgeResult.output,
            runtime_ms: judgeResult.runtime_ms,
            test_results: judgeResult.results ?? [],
    ...
    fastify.get('/submissions/:id', {
        ...
        const { userId } = req;
        const { id } = req.params;
        const result = await fastify.db.query(`SELECT id, status, output, runtime_ms, test_results, xp_awarded, created_at
       FROM submissions WHERE id = $1 AND user_id = $2`, [id, userId]);
