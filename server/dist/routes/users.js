...
// ── Solved stats ───────────────────────────────────────────────────────────
async function getSolvedStats(db, userId) {
    const result = await db.query(`SELECT p.difficulty, p.problem_type, COUNT(DISTINCT s.problem_id) AS cnt
     FROM submissions s
     JOIN problems p ON p.id = s.problem_id
     WHERE s.user_id = $1 AND s.status = 'accepted'
     GROUP BY p.difficulty, p.problem_type`, [userId]);
    const byDiff = {};
    const byType = {};
    let total = 0;
...
async function findRivals(db, userId, skillTier, interests) {
    if (interests.length === 0)
        return 0;
    const placeholders = interests.map((_, i) => `$${i + 3}`).join(', ');
    const result = await db.query(`SELECT u.id, COUNT(ui.interest) AS shared
     FROM users u
     ...
     LIMIT 5`, [userId, skillTier, ...interests]);
    for (const row of result.rows) {
        const [a, b] = [userId, row.id].sort();
        await db.query(`INSERT INTO rivals (user_a, user_b) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [a, b]);
    }
...
async function userRoutes(fastify) {
    ...
    fastify.get('/check-username', async (req, reply) => {
        const { username } = req.query;
        if (!username || username.length < 3)
            return reply.send({ available: false });
        const result = await fastify.db.query('SELECT id FROM users WHERE username = $1', [username.toLowerCase()]);
    ...
    fastify.post('/onboard', async (req, reply) => {
        ...
        const tier = skill_tier ?? req.body.level;
        ...
        try {
            ...
            const userResult = await client.query(`INSERT INTO users (email, password_hash, username, niche, bio, skill_tier, xp)
           VALUES ($1, $2, $3, $4, $5, $6, 0)
           RETURNING id, username, skill_tier, xp`, [email.toLowerCase(), password_hash, username.toLowerCase(), niche ?? null, bio ?? null, tier]);
            ...
            if (socials) {
                for (const [platform, connected] of Object.entries(socials)) {
                    if (connected)
                        await client.query(`INSERT INTO social_connections (user_id, platform, connected) VALUES ($1, $2, $3)
               ON CONFLICT (user_id, platform) DO UPDATE SET connected = $3`, [user.id, platform, connected]);
                }
            }
    ...
    fastify.post('/login', async (req, reply) => {
        ...
        const result = await fastify.db.query(`SELECT id, username, skill_tier, xp, password_hash FROM users WHERE email = $1`, [email.toLowerCase()]);
    ...
    fastify.get('/me', { preHandler: [fastify.authenticate] }, async (req, reply) => {
        const { userId } = req;
        const userResult = await fastify.db.query(`SELECT id, username, niche, bio, skill_tier, xp, created_at FROM users WHERE id = $1`, [userId]);
    ...
    fastify.get('/rivals', { preHandler: [fastify.authenticate] }, async (req, reply) => {
        const { userId } = req;
        const result = await fastify.db.query(`SELECT
    ...
    fastify.post('/xp', { preHandler: [fastify.authenticate] }, async (req, reply) => {
        const { userId } = req;
        ...
        const result = await fastify.db.query(`UPDATE users
         SET xp = GREATEST(0, xp + $1)
         WHERE id = $2
         RETURNING xp`, [Math.round(delta), userId]);
        if (result.rows.length === 0) {
            return reply.status(404).send({ error: 'User not found' });
        }
        const newXp = result.rows[0].xp;
        const newRank = xpToRank(newXp);
        fastify.log.info({ userId, delta, reason: reason ?? 'unspecified', newXp, newRank }, 'XP updated');
    ...
    fastify.patch('/me', { preHandler: [fastify.authenticate] }, async (req, reply) => {
        const { userId } = req;
        const { username, niche, bio, skill_tier, interests, goals, } = req.body;
        const client = await fastify.db.connect();
        try {
            await client.query('BEGIN');
            ...
            if (username) {
                ...
                const clash = await client.query('SELECT id FROM users WHERE username = $1 AND id != $2', [username.toLowerCase(), userId]);
           ...
           WHERE id = $5`, [username?.toLowerCase() ?? null, niche ?? null, bio ?? null, skill_tier ?? null, userId]);
            if (interests) {
                await client.query('DELETE FROM user_interests WHERE user_id = $1', [userId]);
                for (const interest of interests) {
                    await client.query('INSERT INTO user_interests (user_id, interest) VALUES ($1, $2)', [userId, interest]);
                }
            }
            if (goals) {
                await client.query('DELETE FROM user_goals WHERE user_id = $1', [userId]);
                for (const goal of goals) {
                    await client.query('INSERT INTO user_goals (user_id, goal) VALUES ($1, $2)', [userId, goal]);
                }
            }
            await client.query('COMMIT');
            ...
            return reply.send({
                ...
                goals: goalsRes.rows.map((r) => r.goal),
