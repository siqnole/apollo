...
const judgeEnhanced_1 = require("../services/judgeEnhanced");
const testCases = [
    // Basic exact matches
...
function runTests() {
    ...
    for (const testCase of testCases) {
        const result = (0, judgeEnhanced_1.judgeFillBlankEnhanced)(testCase.actual, testCase.expected);
        const testPassed = testCase.shouldPass === (result.status === 'accepted');
        if (testPassed) {
            ...
            console.log(`❌ ${testCase.name}`);
            console.log(`   Expected: "${testCase.expected}", Got: "${testCase.actual}"`);
            console.log(`   Should pass: ${testCase.shouldPass}, Result: ${result.status}`);
            failed++;
...
function testValueExtraction() {
    ...
    for (const test of extractionTests) {
        const extracted = (0, judgeEnhanced_1.extractValue)(test.input);
