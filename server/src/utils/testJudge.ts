...
function generateEdgeCases(
  ...
  const variants: EdgeCaseVariant[] = [
...
function testVariant(
  ...
  problemType: string,
  problemSlug: string
): { variant: string; result: JudgeResult; passed: boolean } {
  let result: JudgeResult;

  if (problemType === 'fill_blank') {
    result = judgeFillBlank(answer, expected);
  } else if (problemType === 'multiple_choice') {
    result = judgeMultipleChoice(answer, expected);
  } else if (problemType === 'ordering') {
    try {
      const submitted = JSON.parse(answer);
      const correct = JSON.parse(expected);
      result = judgeOrdering(submitted, correct);
    } catch (e) {
      result = {
  ...
  try {
      ...
      for (const edgeCase of edgeCases) {
        ...
        if (test.passed === edgeCase.shouldPass) {
          ...
          inconsistencies.push(
            `${slug}: "${edgeCase.label}" (${edgeCase.value}) ${direction} [expected: ${edgeCase.shouldPass}]`
          );
        }
      }
    ...
    for (const cat of categories.sort()) {
      ...
      for (const problem of catProblems) {
        const allPassed = problem.tests.every(
          (t) => t.passed === t.expectedPass
        );
        const icon = allPassed ? '✅' : '❌';

        console.log(
          `  ${icon} [${problem.difficulty}] ${problem.slug} (${problem.type})`
        );

        // Show only failed variants
        const failedVariants = problem.tests.filter(
          (t) => t.passed !== t.expectedPass
        );
        if (failedVariants.length > 0) {
          failedVariants.forEach((test) => {
            ...
            console.log(
              `    ❌ "${test.variant}": ${expectedStr} but ${actualStr}`
            );
            console.log(
              `       Input: "${test.input}" | Status: ${test.status}`
            );
          });
        } else if (problem.tests.length > 0) {
          console.log(
            `      All ${problem.tests.length} edge cases behaved as expected`
          );
        }
      }
    }
