...
export default async function adminRoutes(app: FastifyInstance) {
  ...
  app.post('/appeals', async (req: any, reply) => {
    ...
    const { slug, reason, code } = req.body as {
      slug:   string;
      reason: string;
      code?:  string;
    };
    ...
    await app.db.query(
      `INSERT INTO appeals (user_id, slug, reason, code) VALUES ($1, $2, $3, $4)`,
      [userId, slug, reason.trim(), code ?? null]
    );
  ...
  app.get('/appeals/check/:slug', async (req: any) => {
    let userId: string | null = null;
    try {
      await req.jwtVerify();
      userId = req.user?.sub ?? null;
  ...
  app.get('/admin/appeals', { preHandler: requireAdmin }, async (req: any) => {
  ...
  app.patch('/admin/appeals/:id', { preHandler: requireAdmin }, async (req: any, reply) => {
    ...
    if (rowCount === 0) return reply.status(404).send({ error: 'Appeal not found' });
    return { ok: true };
  });

  // ── GET /api/admin/submissions ────────────────────────────────────────────
  app.get('/admin/submissions', { preHandler: requireAdmin }, async (req: any) => {
    const {
      slug, status, user,
      limit  = '50',
      offset = '0',
    ...
    if (slug)   { conditions.push(`p.slug ILIKE $${i++}`);     params.push(`%${slug}%`); }
    if (status) { conditions.push(`s.status = $${i++}`);       params.push(status);       }
    if (user)   { conditions.push(`u.username ILIKE $${i++}`); params.push(`%${user}%`); }
