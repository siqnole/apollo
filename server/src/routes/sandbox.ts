...
async function startContainer(problemSlug: string, problemType: string): Promise<string> {
...
async function execInContainer(
  ...
  try {
    const { stdout, stderr } = await execAsync(
      `docker exec -w ${JSON.stringify(cwd)} -u user ${containerId} bash -c '${safe}'`,
...
async function destroyContainer(containerId: string) {
  await execAsync(`docker rm -f ${containerId}`).catch(() => {});
}

async function checkSolution(containerId: string, problemSlug: string) {
  try {
    const { stdout, exitCode } = await execInContainer(
      containerId,
      `check-solution ${problemSlug}`,
      '/home/user'
    );
...
export default async function sandboxRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/sandbox/start', {
    ...
    const { userId } = req;
    const { problem_slug } = req.body as { problem_slug: string };

    if (!problem_slug) return reply.status(400).send({ error: 'problem_slug required' });

    const result = await fastify.db.query(
      `SELECT id, slug, problem_type FROM problems WHERE slug = $1 AND active = true`,
      [problem_slug]
    );
    if (result.rows.length === 0) return reply.status(404).send({ error: 'Problem not found' });
  ...
  fastify.post('/sandbox/exec', {
    ...
    const { userId } = req;
    const { session_id, command, cwd } = req.body as {
      session_id: string;
      command:    string;
      cwd:        string;
    };

    if (!session_id || !command) return reply.status(400).send({ error: 'session_id and command required' });
    ...
    if (cdMatch) {
      ...
      const { stdout, exitCode } = await execInContainer(
        session.containerId, `cd ${JSON.stringify(target)} && pwd`, newCwd
      );
      if (exitCode === 0) newCwd = stdout.trim();
      return reply.send({
        stdout: '',
    ...
    const { stdout, stderr, exitCode } = await execInContainer(
      session.containerId, command, newCwd
    );
  ...
  fastify.post('/sandbox/check', {
    ...
    const { userId } = req;
    const { session_id, problem_slug } = req.body as { session_id: string; problem_slug: string };
    ...
    if (passed) {
      const problemResult = await fastify.db.query(
        `SELECT id, xp_reward FROM problems WHERE slug = $1`, [problem_slug]
      );
      ...
      if (prev.rows.length === 0) {
        ...
        await fastify.db.query(
          `INSERT INTO submissions (user_id, problem_id, problem_type, status, xp_awarded)
           VALUES ($1, $2, $3, 'accepted', $4)`,
          [userId, problem.id, session.problemType, xpAwarded]
        );
      }
